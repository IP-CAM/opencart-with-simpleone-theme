...
class ControllerVendorLtsOption extends Controller {
  ...
  public function add() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      $this->model_vendor_lts_option->addOption($this->request->post);
      ...
      $this->response->redirect($this->url->link('vendor/lts_option'));
    }
  ...
  public function edit() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      $this->model_vendor_lts_option->editOption($this->request->get['option_id'], $this->request->post);
  ...
  public function delete() {
    ...
    if (isset($this->request->post['selected']) && $this->validateDelete()) {
      foreach ($this->request->post['selected'] as $option_id) {
        $this->model_vendor_lts_option->deleteOption($option_id);
      }
  ...
  protected function getList() {
    ...
    $data['breadcrumbs'][] = array(
        ...
        'href' => $this->url->link('vendor/lts_option')
    );

    $data['add'] = $this->url->link('vendor/lts_option/add');
    $data['delete'] = $this->url->link('vendor/lts_option/delete');
    ...
    $option_total = $this->model_vendor_lts_option->getTotalOptions();

    $results = $this->model_vendor_lts_option->getOptions($filter_data);
    ...
    foreach ($results as $result) {
      $data['options'][] = array(
          ...
          'edit' => $this->url->link('vendor/lts_option/edit', '&option_id=' . $result['option_id'])
      );
    }
    ...
    $data['sort_name'] = $this->url->link('vendor/lts_option', '&sort=od.name' . $url, true);
    $data['sort_sort_order'] = $this->url->link('vendor/lts_option', '&sort=o.sort_order' . $url, true);
    ...
    $pagination->url = $this->url->link('vendor/lts_option', '&page={page}', true);
  ...
  protected function getForm() {
    ...
    if (!isset($this->request->get['option_id'])) {
      $data['action'] = $this->url->link('vendor/lts_option/add');
    } else {
      $data['action'] = $this->url->link('vendor/lts_option/edit', '&option_id=' . $this->request->get['option_id']);
    }

    $data['cancel'] = $this->url->link('vendor/lts_option');

    if (isset($this->request->get['option_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      $option_info = $this->model_vendor_lts_option->getOption($this->request->get['option_id']);
    }
    ...
    if (isset($this->request->post['option_description'])) {
      ...
      $data['option_description'] = $this->model_vendor_lts_option->getOptionDescriptions($this->request->get['option_id']);
    } else {
    ...
    if (isset($this->request->post['option_value'])) {
      ...
      $option_values = $this->model_vendor_lts_option->getOptionValueDescriptions($this->request->get['option_id']);
    } else {
      $option_values = array();
    }
  ...
  protected function validateDelete() {
    ...
    foreach ($this->request->post['selected'] as $option_id) {
      $product_total = $this->model_vendor_lts_product->getTotalProductsByOptionId($option_id);
  ...
  public function autocomplete() {
    ...
    if (isset($this->request->get['filter_name'])) {
      ...
      $options = $this->model_vendor_lts_option->getOptions($filter_data);

      foreach ($options as $option) {
        $option_value_data = array();

        if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
          $option_values = $this->model_vendor_lts_option->getOptionValues($option['option_id']);

          foreach ($option_values as $option_value) {
            if (is_file(DIR_IMAGE . $option_value['image'])) {
              $image = $this->model_vendor_lts_image->resize($option_value['image'], 50, 50);
            } else {
              $image = $this->model_vendor_lts_image->resize('no_image.png', 50, 50);
            }
