...
class ControllerVendorLtsRecurring extends Controller {
  ...
  public function index() {
    ...
    $this->load->language('vendor/lts_recurring');

    $this->document->setTitle($this->language->get('heading_title'));

    $this->load->model('vendor/lts_recurring');
  ...
  public function add() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      $this->model_vendor_lts_recurring->addRecurring($this->request->post);
      ...
      $this->response->redirect($this->url->link('vendor/lts_recurring', 'user_token=' . $this->session->data['user_token'] . $url, true));
    }
  ...
  public function edit() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      $this->model_vendor_lts_recurring->editRecurring($this->request->get['recurring_id'], $this->request->post);
      ...
      $this->response->redirect($this->url->link('vendor/lts_recurring'));
    }
  ...
  public function delete() {
    ...
    if (isset($this->request->post['selected']) && $this->validateDelete()) {
      foreach ($this->request->post['selected'] as $recurring_id) {
        $this->model_vendor_lts_recurring->deleteRecurring($recurring_id);
      }
  ...
  public function copy() {
    ...
    if (isset($this->request->post['selected'])) {
      foreach ($this->request->post['selected'] as $recurring_id) {
        $this->model_vendor_lts_recurring->copyRecurring($recurring_id);
      }
  ...
  protected function getList() {
    ...
    $data['breadcrumbs'][] = array(
        ...
        'href' => $this->url->link('vendor/lts_recurring')
    );

    $data['add'] = $this->url->link('vendor/lts_recurring/add');
    $data['copy'] = $this->url->link('vendor/lts_recurring/copy');
    $data['delete'] = $this->url->link('vendor/lts_recurring/delete');
    ...
    $recurring_total = $this->model_vendor_lts_recurring->getTotalRecurrings();

    $results = $this->model_vendor_lts_recurring->getRecurrings($filter_data);
    ...
    foreach ($results as $result) {
      $data['recurrings'][] = array(
          ...
          'edit' => $this->url->link('vendor/lts_recurring/edit', '&recurring_id=' . $result['recurring_id'] . $url, true)
      );
    }
    ...
    $data['sort_name'] = $this->url->link('vendor/lts_recurring', '&sort=pd.name' . $url);
    $data['sort_sort_order'] = $this->url->link('vendor/lts_recurring', '&sort=p.sort_order' . $url);
    ...
    $pagination->url = $this->url->link('vendor/lts_recurring', '&page={page}', true);
    ...
    $this->response->setOutput($this->load->view('vendor/lts_recurring_list', $data));
  }
  ...
  protected function getForm() {
    ...
    if (!isset($this->request->get['recurring_id'])) {
      $data['action'] = $this->url->link('vendor/lts_recurring/add');
    } else {
      $data['action'] = $this->url->link('vendor/lts_recurring/edit', '&recurring_id=' . $this->request->get['recurring_id'] . $url, true);
    }

    $data['cancel'] = $this->url->link('vendor/lts_recurring');

    if (isset($this->request->get['recurring_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      $recurring_info = $this->model_vendor_lts_recurring->getRecurring($this->request->get['recurring_id']);
    }
    ...
    if (isset($this->request->post['recurring_description'])) {
      ...
      $data['recurring_description'] = $this->model_vendor_lts_recurring->getRecurringDescription($recurring_info['recurring_id']);
    } else {
    ...
    $this->response->setOutput($this->load->view('vendor/lts_recurring_form', $data));
  }
  ...
  protected function validateDelete() {
    ...
    foreach ($this->request->post['selected'] as $recurring_id) {
      $product_total = $this->model_vendor_lts_product->getTotalProductsByProfileId($recurring_id);
