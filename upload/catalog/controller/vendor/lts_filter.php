...
class ControllerVendorLtsFilter extends Controller {

  private $error = array();

  public function index() {
    $this->load->language('vendor/lts_filter');
  ...
  public function add() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      $this->model_vendor_lts_filter->addFilter($this->request->post);
      ...
      $this->response->redirect($this->url->link('vendor/lts_filter'));
    }
  ...
  public function edit() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
      $this->model_vendor_lts_filter->editFilter($this->request->get['filter_group_id'], $this->request->post);
  ...
  public function delete() {
    ...
    if (isset($this->request->post['selected'])) {
      foreach ($this->request->post['selected'] as $filter_group_id) {
        $this->model_vendor_lts_filter->deleteFilter($filter_group_id);
      }
  ...
  protected function getList() {
    ...
    $data['breadcrumbs'][] = array(
        ...
        'href' => $this->url->link('vendor/lts_filter')
    );

    $data['add'] = $this->url->link('vendor/lts_filter/add');
    $data['delete'] = $this->url->link('vendor/lts_filter/delete');
    ...
    $filter_total = $this->model_vendor_lts_filter->getTotalFilterGroups();

    $results = $this->model_vendor_lts_filter->getFilterGroups($filter_data);
    ...
    foreach ($results as $result) {
      $data['filters'][] = array(
          ...
          'edit' => $this->url->link('vendor/lts_filter/edit', '&filter_group_id=' . $result['filter_group_id'] . $url, true)
      );
    }
    ...
    $data['sort_name'] = $this->url->link('vendor/lts_filter', '&sort=fgd.name' . $url, true);
    $data['sort_sort_order'] = $this->url->link('vendor/lts_filter', '&sort=fg.sort_order' . $url, true);
    ...
    $pagination->url = $this->url->link('vendor/lts_filter', '&page={page}', true);
    ...
    $this->response->setOutput($this->load->view('vendor/lts_filter_list', $data));
  }
  ...
  protected function getForm() {
    ...
    if (!isset($this->request->get['filter_group_id'])) {
      $data['action'] = $this->url->link('vendor/lts_filter/add');
    } else {
      $data['action'] = $this->url->link('vendor/lts_filter/edit', '&filter_group_id=' . $this->request->get['filter_group_id'] . $url, true);
    }

    $data['cancel'] = $this->url->link('vendor/lts_filter');

    if (isset($this->request->get['filter_group_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      $filter_group_info = $this->model_vendor_lts_filter->getFilterGroup($this->request->get['filter_group_id']);
    }
    ...
    if (isset($this->request->post['filter_group_description'])) {
      ...
      $data['filter_group_description'] = $this->model_vendor_lts_filter->getFilterGroupDescriptions($this->request->get['filter_group_id']);
    } else {
    ...
    if (isset($this->request->post['filter'])) {
      ...
      $data['filters'] = $this->model_vendor_lts_filter->getFilterDescriptions($this->request->get['filter_group_id']);
    } else {
      $data['filters'] = array();
    }
    ...
    $this->response->setOutput($this->load->view('vendor/lts_filter_form', $data));
  }
  ...
  public function autocomplete() {
    ...
    if (isset($this->request->get['filter_name'])) {
      ...
      $filters = $this->model_vendor_lts_filter->getFilters($filter_data);
