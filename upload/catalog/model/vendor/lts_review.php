...
class ModelVendorLtsReview extends Model {
	public function addReview($data) {
		...
		$this->db->query("INSERT INTO ". DB_PREFIX ."lts_vendor_review SET review_id= '". (int)$review_id ."', product_id = '". (int)$data['product_id'] ."', vendor_id='". (int)$this->customer->getId() ."', firstname = '". $this->db->escape($this->customer->getFirstName()) ."', lastname = '". $this->db->escape($this->customer->getLastName()) ."'");
	...
	public function deleteReview($review_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "lts_vendor_review WHERE review_id = '" . (int)$review_id . "' AND vendor_id='". (int)$this->customer->getId() ."'");
	...
	public function getTotalReviews($data = array()) {
		...
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r LEFT JOIN " . DB_PREFIX . "product_description pd ON (r.product_id = pd.product_id) RIGHT JOIN ". DB_PREFIX ."lts_vendor_review lvr ON (lvr.review_id = r.review_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND lvr.vendor_id='". (int)$this->customer->getId() ."'";
