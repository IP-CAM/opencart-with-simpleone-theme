...
class ControllerVendorMail extends Controller {
	...
	public function add() {
		...
		if(($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if($this->request->post['status'] == '1') {
				...
				if($this->request->post['too'] == 'approval_vendor') {
				...
				} elseif($this->request->post['too'] == 'napproval_vendor') {
					...
					// all vendor
				} elseif($this->request->post['too'] == 'all_vendor') {
					$emails = $this->model_vendor_lts_mail->nonapprovalVendor();
	...
	protected function getList() {
		...
		foreach ($mails as $value) {
			$name = $this->model_vendor_lts_mail->getTooName($value['too_id']);
			$data['mails'][] = array(
